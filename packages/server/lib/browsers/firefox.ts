...
export async function open (browser: Browser, url, options: any = {}): Promise<BrowserInstance> {
  ...
  if (!await fs.pathExists(path.join(userCSSPath, 'userChrome.css'))) {
    ...
    //
    // TODO: allow configuring launchOptionsthe launchOptions.firefox will only exist in firefox to allow modifying userCss
    const userCss = options.isHeadless ? FIREFOX_HEADLESS_USERCSS : FIREFOX_HEADED_USERCSS
